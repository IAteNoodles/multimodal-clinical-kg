...
import argparse, torch, gc, json, os, sys, shutil
...
def load_checkpoint(path, model, device):
    ...
    if model_path.exists():
        ...
        # Load full ckpt to CPU, copy model weights to GPU, discard rest
...
def train(args):
    ...
    if ckpt_dir and args.resume:
        ...
        if latest_path.exists():
            start_ep, global_step, metrics = load_checkpoint(latest_path, model, device)
    ...
    for ep in range(start_ep + 1, args.epochs + 1):
            ...
            with torch.amp.autocast('cuda', enabled=scaler is not None):
                if _needs_modality_ids(model):
                    ...
                    neg = model.score(neg_all[:, 0], neg_all[:, 1], neg_all[:, 2],
                        entity_type_ids, entity_modality_ids).reshape(n_pos, -1)
                else:
                    pos = model.score(h, r, t)
                    neg = model.score(neg_all[:, 0], neg_all[:, 1], neg_all[:, 2]).reshape(n_pos, -1)
        ...
        if ckpt_dir:
            ...
            save_checkpoint(ckpt_dir / "latest.pt", model, opt, sched, scaler, global_step, ep, ep_metrics, skip_full=(args.keep_last_n == 0))
