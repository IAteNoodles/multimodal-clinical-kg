...
def train(args):
    ...
    if args.warm_start_complex and isinstance(model, MultimodalCASCADEModel):
        ckpt_path = Path(args.warm_start_complex)
        if ckpt_path.suffix == '.pt':
            ...
            if 'entity_embeddings.weight' in sd:
                ...
                if ew.shape != model.entity_embeddings.weight.shape:
                    print(f"  WARNING: entity_embeddings shape mismatch: {ew.shape} vs {model.entity_embeddings.weight.shape}")
                else:
                    model.entity_embeddings.weight.data.copy_(ew)
                    model.entity_embeddings.weight.requires_grad_(False)
                    print(f"  warm-started entity_embeddings from {ckpt_path} (frozen)")
            else:
                print(f"  WARNING: no entity_embeddings.weight in {ckpt_path}, keys={list(sd.keys())[:5]}")
        else:
            print(f"  WARNING: {ckpt_path} not a .pt file, skipping warm-start")
...
if __name__ == '__main__':
    ...
    p.add_argument('--warm-start-complex', type=str, default=None,
                   help='load pretrained ComplEx entity_embeddings and freeze them')
    args = p.parse_args()
    train(args)
